...
import static sad.sras.models.auth.Role.ADMIN;
import static sad.sras.models.auth.Role.ASAD;
import static sad.sras.models.auth.Role.SAD;
import static sad.sras.models.auth.Role.DEPT;
...
import sad.sras.services.auth.LogoutService;
...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		.authorizeHttpRequests(auth -> auth .requestMatchers("/auth/**", "/csrf-token","/booking/hall-allotments**","/room/hall","/offices").permitAll()
				.requestMatchers("/visitor","/visitor/**","/booking/export").hasAnyAuthority(SAD.name(), ASAD.name(), DEPT.name())
				...
				.requestMatchers(POST,"/booking/get-remark","/booking/action","/offices","/booking").hasAnyAuthority(DEPT.name(), ASAD.name())
...
//					y -> y.policyDirectives("default-src 'self'; "
//							//+ "script-src 'self'; "
//							+"frame-src 'self'; connect-src 'self';" + " require-trusted-types-for 'script';"));
	    	x.contentSecurityPolicy(
	    		    y -> y.policyDirectives(
	    		        "default-src 'self'; " +
	    		        "script-src 'self'; " +
	    		        "object-src 'none'; " +
	    		        "frame-src 'self'; " +
