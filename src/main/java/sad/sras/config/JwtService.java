...
import sad.sras.logs.AuditTrail;
...
import sad.sras.repo.auth.InvalidateRepository;
...
public class JwtService {
	...
	public boolean isTokenValid(String token, UserDetails userDetails, String ipAddress) {
		...
		Integer tokenSession = extractSession(token);

		Optional<CurrentUsers> currentUser = currentUsersRepo.findByUsername(username);

		if(currentUser.isPresent()) {
		    if(!tokenSession.equals(currentUser.get().getSession())) {
		        isValid=false; // token invalid (old session)
		    }
		}
	...
	public Integer extractSession(String token) {
	    return extractClaim(token, claims -> claims.get("session", Integer.class));
	}
